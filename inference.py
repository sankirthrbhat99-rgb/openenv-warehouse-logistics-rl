...
def run_inference():
    ...
    try:
        reset_response = requests.post(f"{ENV_URL}/reset")
        if reset_response.status_code != 200:
            print(f"❌ ERROR: /reset failed. Server said: {reset_response.text}")
            return
        state = reset_response.json()
    except requests.exceptions.ConnectionError:
        print("❌ ERROR: Cannot connect. Is your Uvicorn server running in Terminal 1?")
        return
