...
def run_inference():
    ...
    task_names = ["warehouse_1", "warehouse_2", "warehouse_3"]
    ...
    for task_name in task_names:
        ...
        for step in range(25): # Increased to 25 steps to give it time to walk the whole grid
            print(f"\n--- {task_name} Step {step+1} ---", flush=True)
            ...
            if state.get('done'):
                ...
                print(f"[END] task={task_name} score=0.99 steps={step}", flush=True)
                break
            ...
            print(f"[END] task={task_name} score=0.01 steps={25}", flush=True)
